...
const ESTOQUE_DB_NAME = "biServiceEstoquePersistenteDB_v3";
const ESTOQUE_DB_VERSION = 3;
const ESTOQUE_STORE_NAME = "estoqueUltimoUploadStore";
const ESTOQUE_GLOBAL_CACHE_KEY = "__BI_SERVICE_ESTOQUE_ULTIMO_UPLOAD__";
const ESTOQUE_CACHE_RECORD_KEY = "ultimoUploadEstoque_v3";
const ESTOQUE_CACHE_META_KEY = "biServiceEstoqueUltimoUploadMeta_v3";

function getEstoqueGlobalCache() {
  if (typeof window === "undefined") return null;
  return window[ESTOQUE_GLOBAL_CACHE_KEY] || null;
}

function setEstoqueGlobalCache(data) {
  if (typeof window === "undefined") return;
  window[ESTOQUE_GLOBAL_CACHE_KEY] = data;
}
...
async function requestEstoquePersistentStorage() {
...
function saveEstoqueCache(data) {
  return openEstoqueDB().then((db) => {
    return new Promise((resolve, reject) => {
      ...
      const req = store.put(payload, ESTOQUE_CACHE_RECORD_KEY);

      req.onerror = () => {
        console.error("Erro ao salvar cache IndexedDB (estoque)", req.error);
        reject(req.error || new Error("Erro ao salvar cache IndexedDB"));
      };
      ...
      tx.oncomplete = () => {
        ...
        try {
          window.localStorage.setItem(
            ESTOQUE_CACHE_META_KEY,
            JSON.stringify({
              savedAt: payload.savedAt,
              estoqueCount: Array.isArray(payload.estoqueAll) ? payload.estoqueAll.length : 0,
              vendasCount: Array.isArray(payload.vendasAll) ? payload.vendasAll.length : 0,
            })
          );
        } catch {}
        resolve(payload);
      ...
      tx.onerror = () => {
        console.error("Erro na transação IndexedDB (save estoque)", tx.error);
        db.close();
        reject(tx.error || new Error("Erro na transação save estoque"));
      };

      tx.onabort = () => {
        console.error("Transação IndexedDB abortada (save estoque)", tx.error);
        db.close();
        reject(tx.error || new Error("Transação abortada ao salvar estoque"));
      };
    });
  });
}
...
function loadEstoqueCache() {
  return openEstoqueDB().then((db) => {
    return new Promise((resolve, reject) => {
      ...
      const store = tx.objectStore(ESTOQUE_STORE_NAME);
      const req = store.get(ESTOQUE_CACHE_RECORD_KEY);
      ...
      req.onerror = () => {
        console.error("Erro ao ler IndexedDB estoque", req.error);
        db.close();
        reject(req.error || new Error("Erro ao ler IndexedDB estoque"));
      };

      tx.onerror = () => {
        console.error("Erro na transação IndexedDB (load estoque)", tx.error);
        db.close();
        reject(tx.error || new Error("Erro na transação load estoque"));
      };
    });
  });
}
...
const C_CARD_BORDER = "rgba(255,255,255,0.12)";
const C_CARD_BG = "#050505";
...
function Card({ title, borderColor = C_CARD_BORDER, children, right = null }) {
  ...
  return (
    <section
      ...
      style={{ border: `1px solid ${C_CARD_BORDER}`, background: C_CARD_BG, boxShadow: "none" }}
    >
      {hasHeader && (
        ...
        {options.map((opt) => {
          ...
          return (
            <option
              key={val}
              value={val}
              style={{ color: "#fff", backgroundColor: "#171717" }}
            >
              {labelText}
            </option>
          );
...
/* ====== PAGE ====== */
export default function DashboardEstoquePage({ activeSection: activeSectionProp } = {}) {
  const [internalActiveSection, setInternalActiveSection] = useState("resumo");
  const activeSection = activeSectionProp || internalActiveSection;

  useEffect(() => {
    if (typeof window === "undefined" || activeSectionProp) return;
    ...
    function syncSectionFromHash() {
      ...
      if (hash && validTabs.includes(hash)) {
        ...
        setInternalActiveSection("resumo");
      }
    }
  ...
  const restoredEstoqueRef = useRef(false);
...
const [planSkuQuery, setPlanSkuQuery] = useState("");
  ...
  useEffect(() => {
    requestEstoquePersistentStorage();
  }, []);

  // 🔹 Restaura automaticamente o último upload mesmo ao trocar de módulo no BI Service
  useEffect(() => {
    let cancelled = false;

    const applyCachedData = (cached, sourceLabel = "cache") => {
      if (!cached || cancelled) return false;

      const estoqueAll = Array.isArray(cached.estoqueAll) ? cached.estoqueAll : [];
      const vendasAll = Array.isArray(cached.vendasAll) ? cached.vendasAll : [];

      if (!estoqueAll.length && !vendasAll.length) return false;
      ...
      setError("");
      setStatus(sourceLabel === "memoria" ? "Dados mantidos do último upload" : "Dados restaurados do último upload");
      setProgress(100);
      restoredEstoqueRef.current = true;
      return true;
    };

    // 1) Restauração instantânea quando troca entre Consultor/Estoque sem recarregar a página.
    applyCachedData(getEstoqueGlobalCache(), "memoria");

    // 2) Restauração persistente quando fecha/abre navegador ou recarrega a página.
    if (typeof window !== "undefined" && "indexedDB" in window) {
      (async () => {
        try {
          const cached = await loadEstoqueCache();
          applyCachedData(cached, "indexeddb");
        } catch (err) {
          console.error("Erro ao restaurar cache do dashboard de estoque (IndexedDB)", err);
        }
      })();
    }
  ...
  // Mantém uma cópia em memória enquanto o usuário navega entre os módulos do BI Service.
  useEffect(() => {
    if (!allRowsEstoque.length && !salesRowsAll.length) return;
    const payload = {
      estoqueAll: allRowsEstoque,
      vendasAll: salesRowsAll,
      savedAt: new Date().toISOString(),
    };

    setEstoqueGlobalCache(payload);

    if (typeof window !== "undefined" && "indexedDB" in window) {
      saveEstoqueCache(payload).catch((err) => {
        console.error("Erro ao manter cache do dashboard de estoque (IndexedDB)", err);
      });
    }
  }, [allRowsEstoque, salesRowsAll]);
  ...
  async function onDropUpload(e) {
    ...
    if (!ok) {
      setError("Arquivo inválido. Envie uma planilha .xlsx, .xls ou .csv.");
      return;
    }
  ...
  async function onUpload(e) {
    ...
    try {
      ...
      setEstoqueGlobalCache(cachePayload);

      setStatus("Salvando último upload…");
      await saveEstoqueCache(cachePayload);
...
function exportMinimoXlsx() {
  ...
  return (
    ...
    >
      {dragActive && (
        <div
          className="fixed inset-0 z-[99999] bg-black/70 backdrop-blur-sm border-4 border-dashed border-emerald-400 flex items-center justify-center pointer-events-none no-print"
        >
          <div className="text-center rounded-2xl border border-emerald-400/40 bg-black/90 px-10 py-8 shadow-2xl">
...
<SelectDark
                ...
                <div className="flex flex-col">
                  ...
                  <input list="sku-sug-resumo"
                    ...
                    style={{ height: "46px", backgroundColor: "#171717", color: "#ffffff", border: "1px solid rgba(255,255,255,0.14)", outline: "none" }}
                    placeholder="buscar…"
                    value={query}
                    onChange={(e) => {
                    ...
                    <div className="flex items-end gap-2 overflow-x-auto pb-1 mb-2 no-print details-scroll">
                      ...
                      <div className="min-w-[340px] flex flex-col gap-1">
                        ...
                        <input list="sku-sug-vendas"
                          ...
                          style={{ height: "46px", backgroundColor: "#171717", color: "#ffffff", border: "1px solid rgba(255,255,255,0.14)" }}
                          placeholder="buscar..."
