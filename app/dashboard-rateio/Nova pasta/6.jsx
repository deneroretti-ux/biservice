...
async function parseBudgetReceitaXlsx(file) {
  ...
  return {
    year,
    companies: Object.keys(entries).sort(),
    entries,
    filename: file?.name || "",
    companyColumns,
  };
...
function compareBudgetPlanoOrder(a, b) {
  const codeA = budgetPlanoCode(a?.plano ?? a ?? "");
  const codeB = budgetPlanoCode(b?.plano ?? b ?? "");

  if (codeA && codeB) {
    const pa = codeA.split(".").map((x) => Number(x) || 0);
    const pb = codeB.split(".").map((x) => Number(x) || 0);
  ...
  return String(a?.plano ?? a ?? "").localeCompare(String(b?.plano ?? b ?? ""), "pt-BR");
}
...
function buildBudgetEmpresaPayload(receitaData, dreDataList) {
  ...
  for (const item of Object.values(entries).sort((a, b) => {
    const ea = String(a?.empresa || "");
    const eb = String(b?.empresa || "");
  ...
  return {
    ...
    summary: {
      ...
      planos: Array.from(new Set(Object.values(orderedEntries).map((x) => x.plano))),
