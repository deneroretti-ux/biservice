...
const DEFAULT_PLANO_MATCH_MAP = {
  ...
  "mkt reg 06 impressao e producao de material": "10.6.2.2.4 - MKT-REG: 06. Impressão e Produção de material",
  "material promocional": "10.6.1.3 - Brindes e Amostras",
  ...
  "outros impostos e taxas": "19.4 - Outros Impostos e Taxas",
  "desconto sobre boticario franchising": "8.3.2 - TRF Desconto",
  "icms": "8.2.1 - ICMS (ICMS ST - ICMS Próprio)",
  "impressao e producao de material": "10.6.2.2.4 - MKT-REG: 06. Impressão e Produção de material",
...
function loadPlanoMatchMap() {
  return {
    // Primeiro carrega o JSON oficial/base.
    ...planoDeParaOficial,

    // Depois aplica os vínculos corrigidos da page.
    // Assim o JSON antigo não sobrescreve correções novas.
...
function resolvePlanoMatch(planoRateio, drePlanos = [], planoMap = {}) {
  ...
  if (mapped) {
    // Se ainda não há DRE carregado, mantém o vínculo informado.
    if (!drePlanos?.length) {
      return { matched: mapped, confidence: 1, mode: "mapped", normalizedRateio };
    }

    // Mas quando há DRE carregado, valida se o destino existe de fato na base.
    // Isso evita plano antigo do JSON apontando para código que não existe no DRE atual.
    ...
    // Se o destino do mapa não existir no DRE, continua para tentar match exato/fuzzy.
  }
...
export default function DashboardRateioUploadInteligentePage() {
  ...
  const budgetDrePlanos = useMemo(
    () => Array.from(new Set(Object.values(budgetEmpresaData?.entries || {}).sort((a, b) => compareBudgetPlanoOrder(a, b)).map((item) => String(item?.plano ?? "")).filter(Boolean))),
    [budgetEmpresaData]
  );
  ...
  const generatedBudgetMetaForPlano = (plano, empresaFiltro = "Todas", monthKeySet = null) => {
    ...
    for (const item of items) {
      if (item?.calcEligible === false) continue;
      ...
      const manualTotal = getManualBudgetCellValue(budgetManualCells, item?.empresa, plano, "Todos");
      if (manualTotal != null) {
        total += Math.abs(manualTotal);
        foundAny = true;
        continue;
      }

      for (const [mes, mm] of filteredMonths) {
        const manualMes = getManualBudgetCellValue(budgetManualCells, item?.empresa, plano, mes);
...
const clearAllBudgets = () => {
  ...
  const generatedBudgetTotal = useMemo(() => {
    ...
    return total;
  }, [generatedBudgetAvailable, contasNoFiltro, fEmpresa, selectedMonthKeys, budgetEmpresaData, budgetEmpresaIndex, budgetDrePlanos, planoMatchMap, budgetManualCells]);
  ...
  const generatedBudgetByMes = useMemo(() => {
    ...
    for (const item of Object.values(budgetEmpresaData?.entries || {})) {
      ...
      for (const [mes, mm] of Object.entries(item?.months || {})) {
        if (!monthMatchesFilter(mes, selectedMonthKeys)) continue;

        const mesNum = monthNumFromMesKey(mes);
        let previsto = budgetComparableValueFromMonth(mm);

        const manualMes = getManualBudgetCellValue(budgetManualCells, item?.empresa, item?.plano, mes);
        const manualTotal = getManualBudgetCellValue(budgetManualCells, item?.empresa, item?.plano, "Todos");
        if (manualMes != null) previsto = Math.abs(manualMes);
        else if (manualTotal != null) previsto = Math.abs(manualTotal) / Math.max(1, Object.keys(item?.months || {}).length);

        if (!Number.isFinite(previsto)) continue;

        map.set(mes, (map.get(mes) || 0) + previsto);
        map.set(mesNum, (map.get(mesNum) || 0) + previsto);
      }
    }

    return map;
  }, [generatedBudgetAvailable, budgetEmpresaData, fEmpresa, selectedMonthKeys, budgetManualCells]);
  ...
  const budgetEmpresaRows = useMemo(() => {
    ...
    for (const item of Object.values(budgetEmpresaData?.entries || {})) {
      ...
      if (budgetMesFilter !== "Todos") {
        const manual = getManualBudgetCellValue(budgetManualCells, empresa, plano, budgetMesFilter);
        if (manual != null) {
          totalItem = manual;
          manualApplied = true;
        } else {
        ...
        const manualTotal = getManualBudgetCellValue(budgetManualCells, empresa, plano, "Todos");
        if (manualTotal != null) {
          ...
          for (const [mesKey, mm] of Object.entries(item?.months || {})) {
            const manualMes = getManualBudgetCellValue(budgetManualCells, empresa, plano, mesKey);
            if (manualMes != null) {
              totalItem += manualMes;
              manualApplied = true;
            } else {
