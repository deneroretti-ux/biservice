...
function TooltipRich({ active, payload, label, labelPrefix }) {
  ...
  return (
    <div className="rounded-xl border border-white/10 bg-[#0c1118] px-3 py-2 shadow-lg">
      ...
      {(pctLabel || cumPctLabel || raw?.execPctLabel) && (
        <div className="mt-2 pt-2 border-t border-white/10 text-xs text-white/60 flex gap-3">
          {raw?.execPctLabel ? (
            <div>
              Execução: <span className="text-white/80">{raw.execPctLabel}</span>
            </div>
          ) : pctLabel ? (
            <div>
...
const BUDGET_MANUAL_CELLS_LS_KEY = "bi_service_rateio_budget_manual_cells_v1";
...
function buildBudgetEmpresaPayload(receitaData, dreDataList) {
  ...
  for (let dreIdx = 0; dreIdx < (dreDataList || []).length; dreIdx++) {
    ...
    const receitaByMonth = receitaCompanyKey ? receitaData?.entries?.[receitaCompanyKey] : null;

    companyLinks.push({
      dreEmpresa: company,
      receitaEmpresa: receitaCompanyKey || "",
      matched: !!receitaByMonth,
...
export default function DashboardRateioUploadInteligentePage() {
  ...
  function getBudgetItemsForMatchedPlano(matchedPlano) {
    ...
    if (code) {
      ...
      const children = all.filter((item) => {
        const itemCode = budgetPlanoCode(item?.plano);
        return itemCode && itemCode.startsWith(code + ".") && item?.calcEligible !== false;
      });
  ...
  function getDirectManualOverrideForPlano(planoRateio, empresaFiltro = "Todas", monthKeySet = null, match = null) {
    const manualMap = budgetManualCells || {};
    const planoCandidates = [planoRateio, match?.matched].filter(Boolean);
    const normCandidates = planoCandidates.map((p) => normalizePlanoBudgetKey(p)).filter(Boolean);

    let total = 0;
    let found = false;

    for (const [rawKey, rawValue] of Object.entries(manualMap)) {
      ...
      const empKey = parts[0];
      const planoKey = parts.slice(1, -1).join("|");
      const mesKey = parts[parts.length - 1];

      if (!budgetEmpresaMatchesFilter(empKey, empresaFiltro)) continue;

      const normPlanoKey = normalizePlanoBudgetKey(planoKey);
      const samePlano = normCandidates.some((n) => {
        if (!n || !normPlanoKey) return false;
        if (n === normPlanoKey) return true;
        if (n.includes(normPlanoKey) || normPlanoKey.includes(n)) return true;

        const a = new Set(n.split(" ").filter((t) => t.length >= 3));
        const b = new Set(normPlanoKey.split(" ").filter((t) => t.length >= 3));
        if (!a.size || !b.size) return false;

        let common = 0;
        for (const t of a) if (b.has(t)) common++;
        return common / Math.max(a.size, b.size) >= 0.55;
      });

      if (!samePlano) continue;

      if (mesKey === "Todos") {
        // Override em Todos substitui o total do período.
        if (!monthKeySet || !monthKeySet.size) {
          total += Math.abs(value);
        } else {
          // Se houver filtro de mês, distribui o total entre 12 meses.
          total += Math.abs(value) / 12 * monthKeySet.size;
        }
        found = true;
        continue;
      }
      ...
      if (monthKeySet && mesNum && !monthKeySet.has(mesNum)) continue;
  ...
  const generatedBudgetMetaForPlano = (plano, empresaFiltro = "Todas", monthKeySet = null) => {
    ...
    // Prioridade máxima: valor manual digitado no Orçamento.
    // Isso evita o cálculo automático do DRE sobrescrever a edição manual.
    const manualOverride = getDirectManualOverrideForPlano(plano, empresaFiltro, monthKeySet, match);
    if (manualOverride != null) {
      return { valor: manualOverride, estimado: false, manual: true, match };
    }
    ...
    for (const item of items) {
      ...
      const manualTotalFromCell = getManualBudgetCellValueRobusto(budgetManualCells, item?.empresa, manualKeys, "Todos");
      ...
      if (manualTotal != null) {
        ...
        // Valor manual informado em "Todos" é tratado como total do período.
        // Se o dashboard filtrar só alguns meses, distribui proporcionalmente.
      ...
      for (const [mes, mm] of filteredMonths) {
        const manualMes = getManualBudgetCellValueRobusto(budgetManualCells, item?.empresa, manualKeys, mes);
  ...
  async function handleFile(fileOrFiles) {
    ...
    try {
      ...
      if (classified.receita.length || classified.dre.length) {
        const receitaFileLocal = classified.receita[0] || budgetReceitaFile;
        const dreFilesLocal = classified.dre.length ? classified.dre : budgetDreFiles;
...
function openBudgetManager() {
  ...
  function planNamesEquivalent(a, b) {
    const na = normalizePlanoBudgetKey(a);
    const nb = normalizePlanoBudgetKey(b);
    ...
    return common / Math.max(ta.size, tb.size) >= 0.55;
  }

  function updateBudgetBaseManualValue(empresa, plano, mes, value) {
    setBudgetEmpresaData((prev) => {
      const entries = { ...(prev?.entries || {}) };
      let changed = false;

      for (const [entryKey, item] of Object.entries(entries)) {
        if (!budgetEmpresaMatchesFilter(item?.empresa, empresa)) continue;

        const samePlano = planNamesEquivalent(item?.plano, plano);
        if (!samePlano) continue;
        ...
        if (mes === "Todos") {
          if (Number.isFinite(value)) {
            nextItem.manualTotalPrevisto = Math.abs(value);
          } else {
            delete nextItem.manualTotalPrevisto;
          }
        } else {
          const mm = { ...(nextItem.months?.[mes] || {}) };
          if (Number.isFinite(value)) {
            mm.manualPrevisto = Math.abs(value);
            if (Number(mm?.receita || 0)) {
              const receitaAbs = Math.abs(Number(mm.receita || 0));
              mm.pctReceita = receitaAbs ? Math.abs(value) / receitaAbs : Number(mm?.pctReceita || mm?.pct || 0);
              mm.pct = mm.pctReceita;
            }
          } else {
            delete mm.manualPrevisto;
          }
          nextItem.months[mes] = mm;
        }

        entries[entryKey] = nextItem;
        changed = true;
      }

      if (!changed) return prev;

      const nextData = { ...(prev || {}), entries };

      // Salva também a base processada com o valor manual aplicado,
      // para ao voltar do orçamento o dashboard já ler o valor atualizado.
      try {
        if (typeof window !== "undefined") {
          const currentRaw = window.localStorage.getItem(BUDGET_EMPRESA_LS_KEY);
  ...
  function setManualBudgetCell(empresa, plano, mes, rawValue) {
    ...
    setBudgetManualCells((prev) => {
      ...
      try {
        if (typeof window !== "undefined") {
          window.localStorage.setItem(BUDGET_MANUAL_CELLS_LS_KEY, JSON.stringify(next));
        }
      } catch {}
    ...
    updateBudgetBaseManualValue(empresa, plano, mes, Number.isFinite(n) ? round2(n) : NaN);
updateBudgetBaseManualValue(empresa, plano, mes, Number.isFinite(n) ? round2(n) : NaN);
  }
  ...
  function clearManualBudgetCell(empresa, plano, mes) {
    ...
    updateBudgetBaseManualValue(empresa, plano, mes, NaN);
updateBudgetBaseManualValue(empresa, plano, mes, NaN);
  }
  ...
  useEffect(() => {
    ...
    try {
      ...
      const currentRaw = localStorage.getItem(BUDGET_EMPRESA_LS_KEY);
      const current = currentRaw ? JSON.parse(currentRaw) : {};
  ...
  }, [budgetEmpresaData, budgetEmpresaHydrated]);
...
const clearAllBudgets = () => {
  ...
  const statusPlanos = useMemo(() => {
    ...
    return arr;
  }, [contasNoFiltro, generatedBudgetAvailable, fEmpresa, selectedMonthKeys, budgetEmpresaIndex, budgetDrePlanos, planoMatchMap, planoMatchDiagnostics, realMapByPlano, budgets, totalGeral, budgetManualCells]);
  ...
  }, [filtered, generatedBudgetAvailable, budgetEmpresaData, fEmpresa, selectedMonthKeys, budgets, planoMatchMap, budgetManualCells]);
  ...
  const budgetEmpresaRows = useMemo(() => {
    ...
    for (const item of Object.values(budgetEmpresaData?.entries || {})) {
      ...
      if (budgetMesFilter !== "Todos") {
        const manual = getManualBudgetCellValueRobusto(budgetManualCells, empresa, [plano], budgetMesFilter);
        if (manual != null) {
          totalItem = manual;
          manualApplied = true;
        } else {
        ...
        const manualTotal = getManualBudgetCellValueRobusto(budgetManualCells, empresa, [plano], "Todos");
        if (manualTotal != null) {
          ...
          for (const [mesKey, mm] of Object.entries(item?.months || {})) {
            const manualMes = getManualBudgetCellValueRobusto(budgetManualCells, empresa, [plano], mesKey);
            if (manualMes != null) {
              totalItem += manualMes;
              manualApplied = true;
            } else {
    ...
    const allRows = Object.values(map).sort((a, b) => compareBudgetPlanoOrder(a, b));
    return allRows
      .map((r) => ({ ...r, hasChildren: budgetPlanoHasChildren(r, allRows) }))
      .filter((r) => !q || String(r.plano).toLowerCase().includes(q));
  }, [budgetEmpresaData, budgetQuery, budgetEmpresaFilter, budgetMesFilter, budgetManualCells]);
...
<Card
                ...
                <table className="w-full text-[12px]">
                  <thead className="sticky top-0 bg-[#0c1118] border-b border-white/10">
                    <tr className="text-left">
                      ...
                      <th className="p-2 min-w-[120px] text-right">% Receita</th>
                    </tr>
                  </thead>
                  <tbody>
                      ...
                      <tr key={row.plano} className="border-b border-white/5 hover:bg-white/5">
                        <td className="p-2 text-white/85">
                          ...
                          >
                            ...
                            <span className={row.hasChildren ? "font-semibold text-white" : "text-white/85"}>
                              {row.plano}
                            </span>
                          </div>
                        </td>
                        ...
                        <td className={`p-2 text-right font-semibold ${(row.total || 0) < 0 ? "text-rose-200" : "text-emerald-200"}`}>
  ...
  return (
    <div className="min-h-screen bg-[#0c1118] text-white">
      ...
      <main className="max-w-7xl mx-auto px-6 py-6 space-y-4">
        <Card
          title="Carregar arquivos (.xlsx)"
          right={
            rows.length || budgetReceitaFile || budgetDreFiles.length ? (
              <div className="text-[11px] text-white/60 text-right">
                <div className="text-white/80 font-medium">{fileMeta.name || "Upload inteligente"}</div>
        ...
        >
          <div onDrop={onDrop} onDragOver={onDragOver} className="rounded-2xl border border-dashed border-white/15 bg-white/5 p-4">
            <div className="flex flex-col md:flex-row md:items-center gap-3 justify-between">
              <div className="space-y-1">
                <div className="text-sm font-semibold text-white/90">{rows.length || budgetReceitaFile || budgetDreFiles.length ? "Arquivos carregados" : "Nenhum arquivo carregado"}</div>
                <div className="text-[12px] text-white/60">Pode arrastar vários arquivos ou uma pasta. Eu identifico automaticamente Rateio, Receita e DRE(s), carrego o rateio na base principal e processo o orçamento quando Receita + DRE estiverem presentes.</div>
                ...
                {(smartUploadInfo.rateio.length || smartUploadInfo.receita || smartUploadInfo.dres.length) && (
                  <div className="text-[12px] text-white/70 space-y-1">
                    {!!smartUploadInfo.rateio.length && <div>Rateio: {smartUploadInfo.rateio.length} arquivo(s)</div>}
                    {!!smartUploadInfo.receita && <div>Receita: {smartUploadInfo.receita}</div>}
                    {!!smartUploadInfo.dres.length && <div>DRE(s): {smartUploadInfo.dres.length} arquivo(s)</div>}
                  </div>
                )}
