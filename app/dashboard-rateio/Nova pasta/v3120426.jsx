...
const outrosPlanos = useMemo(() => {
  ...
  const executadoStatusPlanos = useMemo(() => {
    ...
    for (const plano of executadoContas) {
      ...
      const previsto = Number(budgetMeta?.valor || 0);
      const st = budgetStatus(real, previsto);
      const match = budgetMeta?.match || null;
      arr.push({
        plano,
        real,
        previsto,
        ...
        statusLabel: st.label,
        statusEmoji: st.key === "ok" ? "🟢" : st.key === "warn" ? "🟡" : st.key === "over" ? "🔴" : "⚪",
        matchedPlano: match?.matched || "",
        matchMode: match?.mode || "unmatched",
    ...
    return arr;
  }, [executadoContas, executadoRealMapByPlano, generatedBudgetAvailable, executadoEmpresa, executadoSelectedMonthKeys, budgetEmpresaIndex, budgetDrePlanos, planoMatchMap, budgets, totalGeral]);
  ...
  function exportExecutadoXlsx() {
    try {
      ...
      XLSX.writeFile(wb, "executado_status_plano_diagnostico.xlsx");
    } catch (e) {
      console.error(e);
      alert("Não foi possível exportar o XLSX.");
    }
                ...
                <tbody>
                  {executadoStatusPlanos.map((p) => (
                    <tr key={p.plano} className="border-t border-white/10 hover:bg-white/5">
                      ...
                      <td className="px-3 py-2 text-white/75">{p.matchedPlano || <span className="text-white/35">—</span>}</td>
                      <td className="px-3 py-2">
                        <span className={`inline-flex rounded-md px-2 py-1 text-[11px] border ${p.matchMode === "mapped" ? "bg-emerald-500/15 border-emerald-400/30 text-emerald-200" : p.matchMode === "exact" ? "bg-sky-500/15 border-sky-400/30 text-sky-200" : p.matchMode === "auto" ? "bg-violet-500/15 border-violet-400/30 text-violet-200" : p.matchMode === "approx" ? "bg-amber-500/15 border-amber-400/30 text-amber-200" : "bg-white/5 border-white/10 text-white/60"}`}>
                          {p.matchMode || "unmatched"}
                        </span>
                      </td>
