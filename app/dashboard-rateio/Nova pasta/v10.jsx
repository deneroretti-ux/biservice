...
export default function DashboardRateioUploadInteligentePage() {
  ...
  const generatedBudgetMetaForPlano = (plano, empresaFiltro = "Todas", monthKeySet = null) => {
    ...
    for (const item of items) {
      ...
      const manualTotal = getManualBudgetCellValueRobusto(budgetManualCells, item?.empresa, manualKeys, "Todos");
      if (manualTotal != null) {
        // Valor manual editado no filtro "Todos" substitui o total do plano.
        // Quando há filtro de mês, distribui proporcionalmente pelos meses disponíveis para não inflar.
        if (monthKeySet && monthKeySet.size) {
          const monthCount = Math.max(1, Object.keys(item?.months || {}).length);
          total += Math.abs(manualTotal) / monthCount;
        } else {
