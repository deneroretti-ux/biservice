...
function normalizeCompanyNameBudget(v) {
  ...
  // Normalização blindada por variações/erros comuns de digitação.
  // Ex.: SHOP, SHOPING, SHOPPING, SHOPPING RIBEIRAO => SHOPPING
  if (txt.includes("SHOP") || txt.includes("SHOPING") || txt.includes("SHOPPING")) return "SHOPPING";

  // Ex.: MONTEAZUL, MONTE AZU, MONT AZUL, MONTE_AZUL => MONTE AZUL
  if (
  ...
  // Ex.: PITANGUEIRAS, PITANGEIRAS, PITANGEURAS, PITANG
  if (
  ...
  // Ex.: VIRADOURO, VIRAD, VIRADRO
  if (
    txt.includes("VIRADOURO") ||
    txt.includes("VIRAD")
  ) return "VIRADOURO";

  // Ex.: COLINA, COL, COLI, COLLINA, COLNA, KOLINA
  if (
    txt.includes("COLINA") ||
    txt.includes("COLI") ||
    txt.includes("COLNA") ||
  ...
  // Ex.: VD, VENDA DIRETA, VENDADIRETA
  if (
    txt === "VD" ||
...
function buildBudgetEmpresaPayload(receitaData, dreDataList) {
  ...
  for (const dre of dreDataList || []) {
    ...
    for (const item of dre.entries || []) {
      ...
      const previsto = receitaLine
        ? receita
        : receita * ((Number(item.valor) || 0) / receita);

      const pct = receitaLine
        ? 1
        : ((Number(item.valor) || 0) / receita);

      if (!previsto && !receitaLine) continue;
...
async function detectUploadKind(file) {
  ...
  try {
    const buf = await file.arrayBuffer();
    const wb = XLSX.read(buf, { type: "array", cellDates: true });
    const sheetNames = (wb.SheetNames || []).map((s) => String(s || "").toUpperCase());
    if (sheetNames.includes("DRE")) return "dre";

    const ws = wb.Sheets[wb.SheetNames?.[0]];
    const matrix = XLSX.utils.sheet_to_json(ws, { header: 1, defval: "", raw: true });

    const firstRows = matrix.slice(0, 20).map((r) => (r || []).map((c) => String(c ?? "").trim()));
    const hasReceitaHeader = firstRows.some((r) => /M[EÊ]S/i.test(String(r?.[0] || "")));
    ...
    const hasMonthCols = (firstRows[0] || []).some((c) => /^\d{2}\/\d{4}$/.test(String(c || "").trim()));
