...
export default function DashboardRateioUploadInteligentePage() {
  ...
  const planoMatchDiagnostics = useMemo(() => {
    ...
    return diagnostics;
  }, [rows, budgetDrePlanos, planoMatchMap, planoResolveCache]);
  ...
  const planoResolveCache = useMemo(() => new Map(), [
    budgetDrePlanos,
    planoMatchMap,
  ]);
  ...
  const resolvePlanoMatchCached = (plano) => {
    ...
    const cached = planoResolveCache.get(key);
    if (cached) return cached;

    const result = resolvePlanoMatch(plano, budgetDrePlanos, planoMatchMap);
    planoResolveCache.set(key, result);
    return result;
  };
