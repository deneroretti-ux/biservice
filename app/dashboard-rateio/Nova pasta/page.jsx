...
function getManualBudgetCellValueSeguro(manualMap, empresa, planos = [], mes = "Todos") {
  ...
  for (const p of planos || []) {
    ...
    const candidates = [
      String(p),
      normalizePlanoBudgetKey(p),
    ].filter(Boolean);

    for (const plano of candidates) {
      const k1 = manualBudgetCellKey(empresa, plano, mes);
      const k2 = manualBudgetNormalizedKey(empresa, plano, mes);
...
export default function DashboardRateioUploadInteligentePage() {
  ...
  }, [budgetManualCells, manualBudgetVersion]);
  ...
  const generatedBudgetMetaForPlano = (plano, empresaFiltro = "Todas", monthKeySet = null) => {
    ...
    // Manual override seguro:
    // Só considera o nome do plano do rateio e o plano DRE vinculado.
    // Não varre a base toda para não distorcer o previsto total.
    const manualCandidates = [plano, match?.matched].filter(Boolean);

    if (match?.matched && match?.mode !== "approx") {
      for (const empresaItem of Array.from(new Set(Object.values(budgetEmpresaData?.entries || {}).map((x) => x?.empresa).filter(Boolean)))) {
        if (!budgetEmpresaMatchesFilter(empresaItem, empresaFiltro)) continue;

        const manualTotal = getManualBudgetCellValueSeguro(budgetManualCells, empresaItem, manualCandidates, "Todos");
        if (manualTotal != null) {
          const selectedCount = monthKeySet && monthKeySet.size ? monthKeySet.size : 12;
          const useFullTotal = !monthKeySet || selectedCount >= 12;
          return {
            valor: useFullTotal ? Math.abs(manualTotal) : (Math.abs(manualTotal) / 12) * selectedCount,
            estimado: false,
            manual: true,
            match,
          };
        ...
        if (monthKeySet && monthKeySet.size) {
          let totalManualMes = 0;
          let foundManualMes = false;
          for (const mesNum of monthKeySet) {
            const manualMes =
              getManualBudgetCellValueSeguro(budgetManualCells, empresaItem, manualCandidates, mesNum) ??
              getManualBudgetCellValueSeguro(budgetManualCells, empresaItem, manualCandidates, `${budgetEmpresaData?.year || new Date().getFullYear()}-${mesNum}`);
            if (manualMes != null) {
              totalManualMes += Math.abs(manualMes);
              foundManualMes = true;
            }
          }
          if (foundManualMes) {
            return { valor: totalManualMes, estimado: false, manual: true, match };
          }
        }
      }
    }
...
const clearAllBudgets = () => {
  ...
  }, [filtered, generatedBudgetAvailable, budgetEmpresaData, fEmpresa, selectedMonthKeys, budgets, planoMatchMap, budgetManualCells, manualBudgetVersion]);
...
const outrosPlanos = useMemo(() => {
  ...
  }, [budgetEmpresaData, budgetQuery, budgetEmpresaFilter, budgetMesFilter, budgetManualCells, manualBudgetVersion]);
