...
const TV_FILTERS_LS_KEY = "bi_service_rateio_tv_filters_v1";
...
function getDashboardBootParams() {
  if (typeof window === "undefined") return { tvMode: false };
  try {
    const sp = new URLSearchParams(window.location.search);
    return { tvMode: sp.get("tv") === "1" || sp.get("modo") === "tv" };
  } catch { return { tvMode: false }; }
}

function readTvFilters() {
  if (typeof window === "undefined") return null;
  try {
    const raw = window.localStorage.getItem(TV_FILTERS_LS_KEY);
...
export default function DashboardRateioUploadInteligentePage() {
  ...
  const [tvLastRefresh, setTvLastRefresh] = useState(() => new Date());

  useEffect(() => {
    try {
      setTvMode(getDashboardBootParams().tvMode);
    } catch {
      setTvMode(false);
    }
  }, []);
  ...
  useEffect(() => {
    const reloadManualBudgetCells = () => {
      try {
        const raw = window.localStorage.getItem(BUDGET_MANUAL_CELLS_LS_KEY);
        const parsed = raw ? JSON.parse(raw) : {};
        setBudgetManualCells(parsed && typeof parsed === "object" ? parsed : {});
        setManualBudgetVersion((v) => v + 1);
      } catch {}
    };

    const onStorage = (event) => {
      if (event?.key === BUDGET_MANUAL_CELLS_LS_KEY || event?.key === "bi_service_budget_refresh_token") {
        reloadManualBudgetCells();
      }
    };

    const onMessage = (event) => {
      if (event?.data?.type === "bi_service_budget_manual_updated") reloadManualBudgetCells();
    };
  ...
  useEffect(() => {
    ...
    const loadTvSavedData = () => {
      try {
        ...
        const budgetRaw = window.localStorage.getItem(BUDGET_EMPRESA_LS_KEY);
        if (budgetRaw) {
          const parsedBudget = JSON.parse(budgetRaw);
          if (parsedBudget?.data) setBudgetEmpresaData(parsedBudget.data);
          if (parsedBudget?.meta?.receitaFile) setBudgetReceitaFile({ name: parsedBudget.meta.receitaFile, __restored: true });
          if (Array.isArray(parsedBudget?.meta?.dreFiles)) {
            setBudgetDreFiles(parsedBudget.meta.dreFiles.map((name) => ({ name, __restored: true })));
          }
          if (parsedBudget?.meta?.manualCells && typeof parsedBudget.meta.manualCells === "object") {
            setBudgetManualCells((prev) => ({ ...(parsedBudget.meta.manualCells || {}), ...(prev || {}) }));
          }
        }

        const manualRaw = window.localStorage.getItem(BUDGET_MANUAL_CELLS_LS_KEY);
        if (manualRaw) {
          const parsedManual = JSON.parse(manualRaw);
          if (parsedManual && typeof parsedManual === "object") setBudgetManualCells(parsedManual);
        }

        setTvLastRefresh(new Date());
      } catch (e) {
        console.error("Erro ao carregar dados do modo TV", e);
      }
    };

    loadTvSavedData();

    const refreshInterval = setInterval(loadTvSavedData, TV_REFRESH_MS);
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      const saved = readTvFilters();
      if (!saved) return;
      try {
        if (saved.fEmpresa) setFEmpresa(saved.fEmpresa);
        if (saved.fPlano) setFPlano(saved.fPlano);
...
useEffect(() => {
  ...
  function persistTvFilters() {
    try {
      localStorage.setItem(
        TV_FILTERS_LS_KEY,
        JSON.stringify({ busca, fEmpresa, fPlano, mesesSel, savedAt: new Date().toLocaleString("pt-BR") })
      );
    } catch {}
  }
  ...
  function openTvMode() {
    try {
      ...
      localStorage.setItem(
        BUDGET_EMPRESA_LS_KEY,
        JSON.stringify({
          data: budgetEmpresaData || { year: null, entries: {}, summary: null },
          meta: {
            receitaFile: budgetReceitaFile?.name || "",
            dreFiles: (budgetDreFiles || []).map((f) => f?.name || String(f)),
            summary: budgetEmpresaData?.summary || null,
      ...
      localStorage.setItem(
        BUDGET_MANUAL_CELLS_LS_KEY,
        JSON.stringify(budgetManualCells || {})
      );

      persistTvFilters();
      localStorage.setItem("bi_service_tv_open_token", String(Date.now()));

      const url = `${window.location.pathname}?tv=1`;
      const win = window.open(url, "_blank");
      try { win?.focus?.(); } catch {}
    } catch (e) {
      console.error("Erro ao abrir modo TV", e);
      try { window.location.href = `${window.location.pathname}?tv=1`; } catch {}
    }
  }
  ...
  function returnToDashboardFromBudget() {
    ...
    try {
      if (window.opener && !window.opener.closed) {
        try { window.opener.postMessage({ type: "bi_service_budget_manual_updated" }, "*"); } catch {}
        try { window.opener.focus(); } catch {}
        try { window.close(); } catch {}
        return;
      }
  ...
  if (executadoOpen) {
    return (
      <div className="min-h-screen bg-[#0c1118] text-white">
        ...
        <main className={tvMode ? "max-w-none mx-auto px-6 py-5 space-y-5 min-h-screen" : "max-w-7xl mx-auto px-6 py-6 space-y-4"}>
        {tvMode && (
          <div className="rounded-2xl border border-emerald-400/20 bg-emerald-500/10 px-5 py-3 flex items-center justify-between gap-4">
            <div>
              <div className="text-lg font-bold text-white">Painel TV ativo</div>
              <div className="text-xs text-white/60">
                Atualização automática a cada 60s • Última leitura: {tvLastRefresh.toLocaleTimeString("pt-BR")}
              </div>
            </div>
            <div className="text-right">
              <div className="text-xs text-white/60">Empresa</div>
              <div className="text-xl font-bold text-emerald-100">{fEmpresa || "Todas"}</div>
            </div>
          </div>
        )}
          <Card title="Filtros">
          ...
          <div onDrop={onDrop} onDragOver={onDragOver} className="rounded-2xl border border-dashed border-white/15 bg-white/5 p-4">
            <div className="flex flex-col md:flex-row md:items-center gap-3 justify-between">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                <button
                  onClick={openTvMode}
                  className="px-3 py-2 rounded-lg bg-emerald-500/20 border border-emerald-400/30 hover:bg-emerald-500/30 text-sm text-emerald-100"
                  disabled={loading || !rows.length}
                  title="Abrir painel em modo TV"
                >
                  Modo TV
                </button>
                <button
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-3">
          ...
          <Card title="Top Planos de Contas" style={{ height: 460 }}>
            <div style={{ height: 390 }}>
              {topPlanos.length ? (
                <ResponsiveContainer width="100%" height={390}>
                  <BarChart data={topPlanos} barGap={-34} margin={{ top: 34, right: 12, left: 0, bottom: 0 }} isAnimationActive={false}>
                    <CartesianGrid stroke="rgba(255,255,255,0.08)" strokeDasharray="3 3" />
                    <XAxis dataKey="plano" hide />
                    <YAxis tick={{ fill: "rgba(255,255,255,0.65)", fontSize: 12 }} />
                    <Tooltip cursor={false} content={<TooltipRich labelPrefix="Categoria" />} />
                    <Bar dataKey="budget" name="Previsto" fill={C_BLUE} fillOpacity={0.18} radius={[10, 10, 0, 0]} barSize={34}  isAnimationActive={false}/>
