...
export default function DashboardRateioUploadInteligentePage() {
  ...
  const generatedBudgetMetaForPlano = (plano, empresaFiltro = "Todas", monthKeySet = null) => {
    const manualOverride = getManualBudgetOverride(plano, budgets);
    if (manualOverride > 0) {
      return {
        valor: manualOverride,
        estimado: false,
        manual: true,
        match: { matched: plano, confidence: 1, mode: "manual", normalizedRateio: normalizePlanoBudgetKey(plano) },
      };
    }
    ...
    for (const item of items) {
      ...
      for (const [mes, mm] of Object.entries(item?.months || {})) {
        ...
        // Regra da comparação Real x Previsto:
        // Previsto do plano = Receita da empresa/mês × percentual do plano no DRE.
        // Para comparação de execução, despesas entram como orçamento positivo.
        const previstoPorPercentual = receitaMes * Math.abs(pctPlanoReceita);

        const valorMes = manualMes != null
          ? Math.abs(manualMes)
          : manualTotal != null
            ? Math.abs(manualTotal) / Math.max(1, Object.keys(item?.months || {}).length)
            : previstoPorPercentual;
...
const clearAllBudgets = () => {
  ...
  const generatedBudgetTotal = useMemo(() => {
    ...
    for (const item of Object.values(budgetEmpresaData?.entries || {})) {
      ...
      for (const [mes, mm] of Object.entries(item?.months || {})) {
        ...
        const previsto = Number(mm?.previsto ?? mm ?? 0);
        if (Number.isFinite(previsto)) total += previsto;
      }
    }
  ...
  const generatedBudgetByMes = useMemo(() => {
    ...
    for (const item of Object.values(budgetEmpresaData?.entries || {})) {
      ...
      for (const [mes, mm] of Object.entries(item?.months || {})) {
        const mesNum = String(mes).split("-")[1] || String(mes).padStart(2, "0");
        if (selectedMonthKeys && !selectedMonthKeys.has(mesNum)) continue;

        const receitaMes = Number(mm?.receita ?? 0) || 0;
        const pctPlanoReceita = Number(mm?.pctReceita ?? mm?.pct ?? 0) || 0;
        const previsto = receitaMes * Math.abs(pctPlanoReceita);
        if (!Number.isFinite(previsto)) continue;

        const valor = previsto;
        map.set(mes, (map.get(mes) || 0) + valor);
        map.set(mesNum, (map.get(mesNum) || 0) + valor);
      }
    }
  ...
  const byMesBudget = useMemo(() => {
    ...
    return byMes.map((it) => {
      ...
      const mesNum = String(it.mes || "").split("-")[1] || String(it.mes || "").padStart(2, "0");
