...
export default function DashboardRateioUploadInteligentePage() {
  ...
  const generatedBudgetMetaForPlano = (plano, empresaFiltro = "Todas", monthKeySet = null) => {
    ...
    // Valor manual tem prioridade, mas somente para o plano exato do rateio
    // ou o plano DRE vinculado. Não varre a base inteira.
    const manualCandidates = [plano, match?.matched].filter(Boolean);
    let manualOverrideTotal = 0;
    ...
    for (const empresaItem of Array.from(new Set(Object.values(budgetEmpresaData?.entries || {}).map((x) => x?.empresa).filter(Boolean)))) {
      ...
      if (manualTotal != null) {
        ...
        manualOverrideTotal += useFullTotal ? Math.abs(manualTotal) : (Math.abs(manualTotal) / 12) * selectedCount;
      ...
      if (monthKeySet && monthKeySet.size) {
        for (const mesNum of monthKeySet) {
          ...
          if (manualMes != null) {
            manualOverrideTotal += Math.abs(manualMes);
            manualOverrideFound = true;
          }
        }
      }
    ...
    if (manualOverrideFound) {
      return { valor: manualOverrideTotal, estimado: false, manual: true, match };
    }
...
function openBudgetManager() {
  ...
  function applyManualBudgetToRuntimeBase(empresa, plano, mes, value) {
    setBudgetEmpresaData((prev) => {
      const entries = { ...(prev?.entries || {}) };
      let changed = false;
      const planoNorm = normalizePlanoBudgetKey(plano);

      for (const [entryKey, item] of Object.entries(entries)) {
        if (!budgetEmpresaMatchesFilter(item?.empresa, empresa)) continue;

        const itemNorm = normalizePlanoBudgetKey(item?.plano);
        const samePlano = itemNorm === planoNorm || itemNorm.includes(planoNorm) || planoNorm.includes(itemNorm);
        ...
        if (mes === "Todos") {
          nextItem.manualTotalPrevisto = Number.isFinite(value) ? Math.abs(value) : null;
          if (!Number.isFinite(value)) delete nextItem.manualTotalPrevisto;
        } else {
          const mm = { ...(nextItem.months?.[mes] || {}) };
          if (Number.isFinite(value)) {
            mm.manualPrevisto = Math.abs(value);
            if (Number(mm?.receita || 0)) {
              mm.pctReceita = Math.abs(value) / Math.abs(Number(mm.receita || 0));
...
applyManualBudgetToRuntimeBase(empresa, plano, mes, Number.isFinite(n) ? round2(n) : NaN);
  }
  ...
  function clearManualBudgetCell(empresa, plano, mes) {
    ...
    applyManualBudgetToRuntimeBase(empresa, plano, mes, NaN);
  }
