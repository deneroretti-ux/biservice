...
useEffect(() => {
  ...
  function openDrillStandalone(plano, mes = "") {
    ...
    try {
      ...
      let baseRows = findRowsForPlano(false, true, true);
      if (!baseRows.length) baseRows = findRowsForPlano(true, true, true);
      if (!baseRows.length) baseRows = findRowsForPlano(false, true, false);
      if (!baseRows.length) baseRows = findRowsForPlano(true, true, false);
      if (!baseRows.length) baseRows = findRowsForPlano(false, false, true);
      if (!baseRows.length) baseRows = findRowsForPlano(true, false, true);
      if (!baseRows.length) baseRows = findRowsForPlano(false, false, false);
      if (!baseRows.length) baseRows = findRowsForPlano(true, false, false);
      ...
      localStorage.setItem(
        ...
        JSON.stringify({
          ...
          rows: baseRows,
          empresa: baseRows.length ? empresaAtual : "Todas",
          mesesSel: baseRows.length ? mesesAtuais : [],
        })
      );
...
const outrosPlanos = useMemo(() => {
  ...
  const drillRowsBase = useMemo(() => {
    ...
    const byPlano = (arr, useEmpresaFilter = false, useMesFilter = false) => {
      if (!Array.isArray(arr) || !arr.length) return [];
      return arr.filter((r) => {
        const planoOk =
          String(r?.plano ?? "").trim() === String(drillPlano ?? "").trim() ||
          normalizePlanoBudgetKey(r?.plano) === alvoPlano;
        if (!planoOk) return false;

        if (useEmpresaFilter && fEmpresa !== "Todas" && String(r?.empresa ?? "").trim() !== String(fEmpresa ?? "").trim()) {
          return false;
        }

        if (useMesFilter && Array.isArray(mesesSel) && mesesSel.length) {
          const mi = mesIndexFromISO(r?.data ?? r?.competencia);
    ...
    const datasetRows = Array.isArray(rows) && rows.length ? rows : readStoredDatasetRows();

    if (standaloneView === "drill") {
      let base = byPlano(detailRows, false, false);

      if (!base.length) {
        try {
          const rawDrill = localStorage.getItem(DRILL_VIEW_LS_KEY);
          const parsedDrill = rawDrill ? JSON.parse(rawDrill) : null;
          if (Array.isArray(parsedDrill?.rows) && parsedDrill.rows.length) {
            base = byPlano(parsedDrill.rows, false, false);
          }
        } catch {}
      }

      if (!base.length) base = byPlano(datasetRows, true, true);
      if (!base.length) base = byPlano(datasetRows, true, false);
      if (!base.length) base = byPlano(datasetRows, false, true);
      if (!base.length) base = byPlano(datasetRows, false, false);

      return base;
    }

    return byPlano(datasetRows, true, true);
