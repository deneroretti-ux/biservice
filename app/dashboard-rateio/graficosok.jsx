  ...
  function openDrillStandalone(plano, mes = "") {
    ...
    try {
      ...
      if (standaloneView === "outros") {
        sourceRows = Array.isArray(detailRows) && detailRows.length
          ? detailRows
          : (Array.isArray(outrosDetalhe?.rows) ? outrosDetalhe.rows : []);
      } else if (standaloneView === "detail") {
        sourceRows = Array.isArray(detailRows) ? detailRows : [];
      } else {
        sourceRows = Array.isArray(filtered) ? filtered : [];
      }

      let drillSnapshotRows = sourceRows.filter(
        (r) => normalizePlanoBudgetKey(r?.plano) === alvoPlano
      );

      if (!drillSnapshotRows.length) {
        drillSnapshotRows = (Array.isArray(rows) ? rows : []).filter(
          (r) => normalizePlanoBudgetKey(r?.plano) === alvoPlano
        );
      }
