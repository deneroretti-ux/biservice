...
const clearAllBudgets = () => {
  ...
  const generatedBudgetByMes = useMemo(() => {
    ...
    const monthList = Array.from(selectedMonthKeys || []).sort();
    ...
    return map;
  }, [generatedBudgetAvailable, contasNoFiltro, fEmpresa, selectedMonthKeys, budgetEmpresaData, budgetEmpresaIndex, budgetDrePlanos, planoMatchMap]);
...
const outrosPlanos = useMemo(() => {
  ...
  const drillByMes = useMemo(() => {
    ...
    // total geral por mês (para orçamento por % no modo manual)
    ...
    return Array.from(m.entries())
      .map(([mes, valor]) => {
        ...
        const previsto = generatedBudgetAvailable
          ? generatedBudgetValueForPlano(drillPlano, fEmpresa, onlyThisMonth)
          : budgetValueForPlanoMes(drillPlano, totalMes, m.size);
        return { mes, valor, previsto, status: budgetStatus(valor, previsto), budget: previsto };
      })
      .sort((a, b) => (a.mes > b.mes ? 1 : -1));
