...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // parse planilha (fora da transação para não segurar o lock)
    const rows = await parseFileToRows(fname, buf);

    // === TRANSACÃO: SUBSTITUIR DADOS DO TENANT E CARREGAR O NOVO LOTE ===
    await prisma.$transaction(async (tx) => {
      // 1) apaga fatos anteriores do tenant (não acumula)
      await tx.fatoConferencia.deleteMany({
        where: { tenantId: tenant.id },
      });

      // (opcional) se quiser limpar uploads antigos do tenant, descomente:
      // await tx.upload.deleteMany({ where: { tenantId: tenant.id } });

      // 2) cria upload "processing"
      ...
      // 3) insere os novos dados em lotes
      const chunk = 500;
      let imported = 0;
    ...
    return NextResponse.json(
      { error: 'Falha no upload/importação' },
      { status: 500 }
    );
  }
}
