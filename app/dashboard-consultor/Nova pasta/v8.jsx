...
function MetricCard({ title, value, subtitle, gaugeColor = COLORS.orange, percent = 0, icon }) {
  ...
  return (
    <Card style={{ padding: 6 }}>
      ...
      <div style={{ fontSize: 10, color: COLORS.subtext, marginBottom: 3, fontWeight: 700 }}>{value}</div>
      ...
      <div style={{ maxWidth: 1440, margin: "0 auto", padding: 6 }}>
        ...
        {!noData && current ? (
          <>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1.25fr 0.85fr", gap: 6, marginBottom: 6 }}>
              <Card style={{ padding: 4 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6, gap: 10, flexWrap: "wrap" }}>
                  <div>
                    <div style={{ fontSize: 13, fontWeight: 900 }}>Ranking de consultores</div>
                    <div style={{ fontSize: 10, color: COLORS.subtext }}>Top 10 • linha tracejada = meta</div>
                  </div>
                ...
                <div style={{ height: rankingChartHeight, overflowY: rankingChartHeight > 360 ? "auto" : "visible" }}>
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={rankingSelectedData} layout="vertical" margin={{ left: 30, right: 10, top: 8, bottom: 8 }} onClick={(state) => {
                      ...
                      <YAxis dataKey="nome" type="category" width={98} tick={{ fontSize: 9 }} />
              ...
              <Card style={{ padding: 16 }}>
                <div style={{ fontSize: 13, fontWeight: 900, marginBottom: 4 }}>Comparativo com a média</div>
                {average ? (
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6 }}>
                    {[
                      ...
                      <div key={label} style={{ background: COLORS.panelAlt, border: `1px solid ${COLORS.border}`, borderRadius: 12, padding: 10, minHeight: 72 }}>
                        <div style={{ fontSize: 10, color: COLORS.subtext, fontWeight: 700, lineHeight: 1.05 }}>{label}</div>
                        <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-start", gap: 2, marginTop: 3 }}>
                          <div style={{ fontWeight: 900, fontSize: 11, color: mineColor, lineHeight: 1.05 }}>{mine}</div>
                          <div style={{ fontSize: 9, fontWeight: 800, color: avgColor, lineHeight: 1.05 }}>Média {avg}</div>
                        </div>
                      </div>
                    ))}
                  </div>
