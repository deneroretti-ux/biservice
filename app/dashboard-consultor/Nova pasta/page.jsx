...
function MetricCard({ title, value, subtitle, gaugeColor = COLORS.orange, percent = 0, icon }) {
  ...
  return (
    <Card style={{ padding: 18 }}>
      ...
      <div style={{ marginTop: 10, fontSize: 13, color: COLORS.subtext, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{subtitle}</div>
    </Card>
  );
}
        ...
        {!noData && current ? (
          <>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(7, minmax(0, 1fr))", gap: 16, marginBottom: 16 }}>
              ...
              <MetricCard title="Boletos" value={formatNumber(current.boletos)} subtitle={current.metas?.b1 ? `B1: ${formatPercent(current.b1Pct)} | Meta: ${formatPercent(current.metas.b1)}` : `B1: ${formatPercent(current.b1Pct)}`} percent={current.metas?.b1 ? Math.min((current.b1Pct / current.metas.b1) * 100, 100) : current.b1Pct * 100} gaugeColor={performanceColor(currentRatios?.b1 || 0)} icon={Target} />
