...
function parseItensBoleto(workbook, fileName) {
  ...
  return items
    ...
    .map((row) => ({
      ...
      boleto1Pct: normalizeRatio(toNumber(row["PARTICIPACAO (%)"] || row["PARTICIPAÇÃO (%)"])),
    }));
}
...
function parseFidelidadeResgate(workbook, fileName) {
  ...
  return items
    ...
    .map((row) => ({
      ...
      monthKey,
      fidelidadeResgatePct: normalizeRatio(toNumber(row["PERIODO ATUAL"] || row["PERÍODO ATUAL"])),
    }));
}
...
function parseMetas(workbook) {
  ...
  return rows
    ...
    .map((row) => ({
      ...
      metaConversao: normalizeRatio(toNumber(row["Meta Conversao"])),
      metaB1: normalizeRatio(toNumber(row["Meta B1"])),
      metaFidelidadePenetracao: normalizeRatio(toNumber(row["Meta Fidelidade Penetracao"])),
      metaFidelidadeResgate: normalizeRatio(toNumber(row["Meta Fidelidade Resgate"])),
      metaTreinamento: normalizeRatio(toNumber(row["Meta Treinamento"])),
    }));
}
...
export default function Page() {
  ...
  const rankingPdvData = useMemo(() => {
    ...
    return Array.from(map.values())
      .map((i) => {
        const ratio =
          metricTab === "score" ? (i.score / i.count) :
          metricTab === "receita" ? safeRatio(i.receita, i.receitaMeta, i.receita > 0 ? 1 : 0) :
          metricTab === "conversao" ? safeRatio(i.conversao / i.count, i.conversaoMeta / i.count, i.conversao > 0 ? (i.conversao / i.count) / 0.19 : 0) :
          safeRatio(i.b1 / i.count, i.b1Meta / i.count, i.b1 > 0 ? (i.b1 / i.count) / 0.30 : 0);
    ...
    return filtered
      .map((item) => {
        const ratio =
          ...
          safeRatio(item.b1Pct || 0, item.metas?.b1 || 0, item.b1Pct ? item.b1Pct / 0.30 : 0);
  ...
  const currentRatios = useMemo(() => {
    ...
    return {
      ...
      b1: safeRatio(current.b1Pct || 0, current.metas?.b1 || 0, current.b1Pct ? current.b1Pct / 0.30 : 0),
    };
  }, [current]);
  ...
  const averageRatios = useMemo(() => {
    ...
    const metaAvg = filtered.length
      ? filtered.reduce((acc, item) => {
          ...
          acc.count += 1;
          return acc;
        }, { receita: 0, ticketMedio: 0, conversao: 0, b1: 0, count: 0 })
      : null;
    ...
    return {
      ...
      b1: safeRatio(average.b1 || 0, metaAvg.b1 / metaAvg.count, average.b1 ? average.b1 / 0.30 : 0),
        ...
        {!noData && current ? (
          <>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(7, minmax(0, 1fr))", gap: 16, marginBottom: 16 }}>
              <MetricCard title="Receita" value={formatCurrency(current.receita)} subtitle={current.metas?.receita ? `Meta consultor: ${formatCurrency(current.metas.receita)}` : `PDV ${current.pdv || "-"}`} percent={current.metas?.receita ? Math.min((current.receita / current.metas.receita) * 100, 100) : current.scorePct} gaugeColor={performanceColor(currentRatios?.receita || 0)} icon={TrendingUp} />
              <MetricCard title="Boletos" value={formatNumber(current.boletos)} subtitle={`B1 calculado: ${formatPercent(current.b1Pct)}`} percent={current.b1Pct * 100} gaugeColor={performanceColor(currentRatios?.b1 || 0)} icon={Target} />
...
<MetricCard
  ...
  gaugeColor={performanceColor(currentRatios?.fidelidadePenetracao || 0)}
  icon={Award}
/>
...
<MetricCard
  ...
  gaugeColor={performanceColor(currentRatios?.fidelidadeResgate || 0)}
  icon={Award}
/>
                  ...
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={rankingType === "consultor" ? rankingData : rankingPdvData} layout="vertical" margin={{ left: 30, right: 10, top: 8, bottom: 8 }}>
                      ...
                      <Bar dataKey="valor" radius={[0, 12, 12, 0]}>
                        {(rankingType === "consultor" ? rankingData : rankingPdvData).map((entry, index) => (
                    ...
                    {[
                      ...
                      <div key={label} style={{ background: COLORS.panelAlt, border: `1px solid ${COLORS.border}`, borderRadius: 14, padding: 14 }}>
                        ...
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 8 }}>
                          <div style={{ fontWeight: 900, fontSize: 18, color: mineColor }}>{mine}</div>
                          <div style={{ fontSize: 13, fontWeight: 800, color: avgColor }}>Média {avg}</div>
                        </div>
                      </div>
                    ))}
                  </div>
