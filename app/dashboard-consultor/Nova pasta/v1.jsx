        ...
        {!noData && current ? (
          <>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(5, minmax(0, 1fr))", gap: 12, marginBottom: 16 }}>
              ...
              <MetricCard title="BP" value={formatPercent(current.bpPct || 0)} subtitle={current.metas?.bp ? `Meta: ${formatPercent(current.metas.bp)}` : "Boleto promocional"} percent={current.metas?.bp ? Math.min(((current.bpPct || 0) / current.metas.bp) * 100, 100) : (current.bpPct || 0) * 500} gaugeColor={performanceColor(currentRatios?.bp || 0)} icon={Target} />
              ...
              <MetricCard title="Penetração Skin" value={formatPercent(current.skinPct || 0)} subtitle={current.metas?.skin ? `Meta: ${formatPercent(current.metas.skin)}` : "Cuidados faciais"} percent={current.metas?.skin ? Math.min(((current.skinPct || 0) / current.metas.skin) * 100, 100) : (current.skinPct || 0) * 500} gaugeColor={performanceColor(currentRatios?.skin || 0)} icon={Award} />
