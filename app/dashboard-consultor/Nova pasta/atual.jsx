    ...
    return Array.from(map.values())
      .map((i) => {
        ...
        const ratio =
          ...
          safeInverseRatio(i.b1 / i.count, i.b1Meta / i.count, i.b1 > 0 ? 0.30 / (i.b1 / i.count) : 0);
        ...
        return {
          ...
          filtroConsultor: "todos",
            ...
            .sort((a, b) => metricTab === "b1" ? a.valor - b.valor : b.valor - a.valor)
      .slice(0, 10);
  }, [filtered, metricTab]);
    ...
    return filtered
      .map((item) => {
        const ratio =
          ...
          safeInverseRatio(item.b1Pct || 0, item.metas?.b1 || 0, item.b1Pct ? 0.30 / item.b1Pct : 0);
        ...
        {!noData && current ? (
          <>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(7, minmax(0, 1fr))", gap: 16, marginBottom: 16 }}>
              ...
              <MetricCard title="Boletos" value={formatNumber(current.boletos)} subtitle={current.metas?.b1 ? `B1: ${formatPercent(current.b1Pct)} | Meta: ${formatPercent(current.metas.b1)}` : `B1: ${formatPercent(current.b1Pct)}`} percent={current.metas?.b1 ? inverseGaugePercent(current.b1Pct, current.metas.b1) : 0} gaugeColor={performanceColor(currentRatios?.b1 || 0)} icon={Target} />
